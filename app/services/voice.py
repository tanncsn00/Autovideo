...
try:
    ...
    def mktimestamp(time_unit):
        hr = int(time_unit / 10_000_000 / 3600)
        mn = int(time_unit / 10_000_000 / 60 % 60)
        sc = int(time_unit / 10_000_000 % 60)
        ms = int(time_unit / 10_000 % 1000)
        return f"{hr:02d}:{mn:02d}:{sc:02d}.{ms:03d}"
    ...
    for i in range(3):
        try:
                ...
                with open(voice_file, "wb") as file:
                    async for chunk in communicate.stream():
                        if chunk["type"] == "audio":
                            ...
                            # Support both old API (create_sub) and new API (feed)
                            if hasattr(sub_maker, 'create_sub'):
                                sub_maker.create_sub(
                                    (chunk["offset"], chunk["duration"]), chunk["text"]
                                )
                            elif hasattr(sub_maker, 'feed'):
            ...
            if not os.path.exists(voice_file) or os.path.getsize(voice_file) == 0:
                logger.warning("failed, audio file is empty")
                continue

            logger.info(f"completed, output file: {voice_file}")
            # Return sub_maker even if no cues — subtitle will fallback to Whisper
            return sub_maker
        except Exception as e:
...
def create_subtitle(sub_maker: submaker.SubMaker, text: str, subtitle_file: str):
    ...
    def formatter(idx: int, start_time: float, end_time: float, sub_text: str) -> str:
        ...
        跑步是一项简单易行的运动
        """
        # Handle both old format (100-nanosecond units) and new format (seconds)
        if isinstance(start_time, float) and start_time < 86400:
            # New API: values in seconds
            def _sec_to_srt(s):
                h = int(s // 3600)
                m = int((s % 3600) // 60)
                sec = int(s % 60)
            ...
            start_t = _sec_to_srt(start_time)
            end_t = _sec_to_srt(end_time)
        else:
            # Old API: values in 100-nanosecond units
    ...
    try:
        # Support both old API (subs/offset) and new API (cues)
        if hasattr(sub_maker, 'cues') and sub_maker.cues:
            ...
            _iter = list(zip(sub_maker.offset, sub_maker.subs))
        else:
            _iter = []

        for _, (offset, sub) in enumerate(_iter):
...
def _get_audio_duration_from_submaker(sub_maker: submaker.SubMaker):
    ...
    # New API: use cues
    if hasattr(sub_maker, 'cues') and sub_maker.cues:
        last = sub_maker.cues[-1]
        return last.end.total_seconds()
    # Old API: use offset
